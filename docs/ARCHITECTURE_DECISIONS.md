...
for notebook kernels is implemented in ADR-019.

---

## ADR-019: ContainerJupyterKernelBackend — OS-level sandbox for Jupyter kernels

**Status:** Accepted
**Date:** 2026-05-29

### Context
`LocalJupyterKernelBackend` runs the ipykernel in the same OS process as the
server. LLM-generated code can traverse the filesystem, make network requests,
and access host secrets. This is the primary remaining gap in the TZ Privacy &
isolation criterion (25%).

### Decision
Implement `ContainerJupyterKernelBackend` so each kernel session runs inside a
Docker container with:
- **No internet access** — `--internal` Docker bridge network (`autovibe-kernels`)
  blocks outbound connections; ZMQ port publishing (5 ports) still allows
  host↔container communication.
- **Read-only rootfs** — `--read-only`; kernel writes only to bind-mounted
  `/workspace` and tmpfs `/tmp`.
- **Minimal privileges** — `--cap-drop ALL`, `--security-opt no-new-privileges`,
  non-root `sandbox` user (from `Dockerfile.sandbox`).
- **Resource caps** — `--memory`, `--cpus`, `--pids-limit`.

**Connection file handshake:**
1. Host picks 5 free ephemeral ports via `_find_free_ports(5)`.
2. Host writes `.kernel.json` (ip=0.0.0.0) to workspace directory.
3. Container starts: `python -m ipykernel_launcher -f /workspace/.kernel.json`.
4. Host `BlockingKernelClient.load_connection_info({...ports, ip="127.0.0.1"})`.
5. `kc.wait_for_ready()` blocks until kernel is responsive.

**Backend selection:** `AUTOVIBE_KERNEL_BACKEND=docker` env var (default: `local`).
`_default_kernel_backend()` in `notebook_env.py` reads this and constructs the
correct backend at `NotebookGymEnv` init time.

### Alternatives considered
- `--network none` — blocks ZMQ port communication entirely; ruled out.
- `--network host` — full host access; no isolation; ruled out.
- `--internal` bridge with port publishing — chosen; blocks egress, allows ZMQ.

### Consequences
- Container kernel sessions provide the same public API as `JupyterKernelSession`.
- `NotebookGymEnv` requires zero changes (backend is injected).
- Shared client methods (`execute_cell`, `inject_bootstrap_context`,
  `dump_variable_to_file`) live in `_KernelClientMixin` to avoid duplication.
- Known limitation: `--pids-limit` still allows process forking within the limit;
  `--internal` network does not restrict container→container communication within
