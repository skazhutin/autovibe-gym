...
| Date | Change |
|------|--------|
| 2026-08-16 | Prepared PR 6 from merged PR #73 (`d1ec571`): added a TMLR-style working manuscript, primary-source bibliography, limitations/disclosures, reproducibility guide, and SHA-256 manifest. Review hardening validates all 13 outputs, binds renderer/hash code, clarifies arm runtimes and the combined 256k cap, and records omitted resource fields as a deviation. A Windows/Linux mismatch exposed by CI was fixed with LF-normalized UTF-8 hashing and comparison plus CRLF regressions; Linux CI then passed `391 passed, 2 deselected`. Current focused suite after the final comparison-only EOL fix: `23 passed`. No submission, public release, raw-run commit, private registry access, or paid API request. |
