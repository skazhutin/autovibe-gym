...
| Date | Change |
|------|--------|
| 2026-06-06 | Post-run self-summary: after every run (gym/fixed/iterative/single/repeated) one extra best-effort LLM call asks the model to summarize its own solution; saved as `run_summary.json` (`gym/run_summary.py`), served via `GET /runs/{id}/summary` + `hasSummary` on `get_run`, and rendered as a card atop the «Мысли» tab — which now shows for every run with a summary, even when thoughts mode is off (old runs without either stay hidden). Privacy preserved: the summarizer only sees the conversation it already had, never the hidden test score |
