...
**Last updated:** 2026-09-11 (Paper V2 storage, protocol provenance, submission-contract, and pandas 3 compatibility hardening committed locally on the M1a-M9 transfer checkpoint; no push, PR, freeze, or experiment activation)
...
| M1b durable candidate bundle | Implemented checkpoint; local hardening committed | Each validated candidate is atomically published to a no-overwrite private `candidate-bundle-v1` directory containing canonical metadata, the serialized model, an immutable notebook snapshot, and `SHA256SUMS`. A candidate-scoped exclusive publication lock now closes the check/replace race between cooperating writers without exposing a partial final directory; stale lock/temp entries remain ignored by restore, and lock-acquisition/publish failures fail closed. A cleanup error after the final directory has already been published no longer reports a false rollback that would orphan valid evidence outside the registry. Both restore and direct verification reject a linked/non-directory store root, and restore rejects a bundle whose bound protocol version differs from the active environment before it can influence incumbent state or public context. Strict schema/JSON/path/file checks, checksum verification before model loading, deterministic registration-index reconstruction, and fail-closed corruption/write failures are covered. Restore deliberately leaves `current=None`, so it cannot bypass clean-run eligibility or trigger hidden evaluation. Public events expose only the schema, candidate ID, and hashes; bundle paths and detailed failures remain private. This is registry recovery, not full kernel/episode resume. |
| M1a+M1b verification | Passing | Fresh local candidate/terminal/finalization suite after the extended storage audit: `30 passed, 3 skipped`; the skips are Windows symlink capabilities. Both candidate and protected-finalization stores passed their real 8-writer race test 20/20 times earlier in the cycle. A broader fresh non-Jupyter Paper V2 contract suite passed `182 passed, 3 skipped`. Four representative notebook integrations passed; the two previously non-reproduced Jupyter terminal/VQ1 targets also passed together `2 passed` in 160.53s. Final full offline suite after all follow-up hardening: `604 passed, 3 skipped, 2 deselected` in 328.31s. Compileall and `git diff --check` pass apart from line-ending notices. No API, paid endpoint, Docker integration, hidden-test research run, push, or PR occurred. |
...
| M2b protected producing-revision finalization | Implemented checkpoint; local hardening committed; not activated | When and only when research mode receives an explicit reserve policy, `submit` and host finalization enter the same host-owned terminal controller. It selects the frozen incumbent regardless of the requested variable, verifies its complete M1b bundle and exact protocol-version binding, executes the immutable notebook snapshot in a restarted kernel under finalization code limits, repeats raw-row/serialization/validation checks, rejects metric drift, atomically stores and reloads a no-overwrite private `protected-finalization-v1` replay artifact under its own candidate-scoped publication lock, and only then spends the final tool unit on the one-shot hidden gate. Missing/corrupt/foreign-protocol incumbents, replay/preflight/drift/storage failures, reserve exhaustion, and hidden failure are terminal with no code repair, live-kernel/autofit fallback, new candidate, resource borrowing, or second hidden attempt. Public receipts expose IDs and hashes only. |
| M2b verification | Passing | Fresh candidate/terminal/finalization suite: `30 passed, 3 skipped`; both no-overwrite stores passed 20/20 concurrent stress runs earlier in the cycle. Direct finalization reads now share the linked-root gate, and a post-publication lock-cleanup failure preserves the already committed artifact result. The protected terminal controller rejects a foreign-protocol bundle before replay, artifact creation, or hidden evaluation. Representative successful replay, historical-only restore, and fail-closed partial-registration notebook paths passed `4 passed` in 26.20s. Broader non-Jupyter Paper V2 contracts passed `182 passed, 3 skipped`; final full offline suite passed `604 passed, 3 skipped, 2 deselected`. Compileall and `git diff --check` pass apart from line-ending notices. Static search still confirms that no runner/CLI constructs `FinalizationReservePolicy`; tests are the only callers selecting fixture reserve values. No API, model endpoint, Docker integration, pilot, experiment, push, or PR occurred. |
...
| M5 symmetric terminal selection | Implemented checkpoint; local hardening committed; not activated | `symmetric-terminal-v1` is the single host-owned selector/finalizer used by protected `NotebookGymEnv` finalization and by the reserve-gated repeated-single-shot path. Both arms select through `CandidateRegistry`, verify the immutable M1b bundle, clean-replay the exact producing revision in a fresh execution state, run the same isolated submission validator, reject validation-metric drift under the same direction/tolerance contract, atomically store/reload the same protected artifact schema, and open the same one-shot `HiddenEvaluationGate`. Process and Docker validation now consistently require a one-dimensional scalar-per-row prediction vector. Hidden evaluation rejects non-finite metrics while permanently consuming its one permitted attempt, so `NaN`/`Inf` cannot be recorded as a successful submission. The repeated adapter stores every admitted attempt as an immutable one-cell notebook bundle, so later failures cannot erase an earlier incumbent and a live `best_model` cannot bypass replay. VQ1 label isolation/query charging/precision are also honored when injected. Existing no-reserve behavior remains on the Paper V1 path. |
| M5 verification | Passing with protocol gate | Fresh submission/terminal/repeated/storage contract group: `53 passed, 3 skipped`; final full offline suite: `604 passed, 3 skipped, 2 deselected` in 328.31s. Regression coverage includes process-backend multi-output rejection, hidden multi-output rejection, all three non-finite metric classes, and one-shot exhaustion after metric failure. Compileall and `git diff --check` pass apart from line-ending notices. An injected reserve still requires explicit `--research-v2-metric-direction` and `--research-v2-score-tolerance`; neither has a default for the active V2 path. Static search confirms no production code constructs `FinalizationReservePolicy` or `ValidationQueryPolicy`, so no runner activates M2/VQ1/M5 automatically. Confirmatory arm symmetry still requires frozen reserve/query/tolerance values, the same Docker execution/prediction configuration, preregistration, and immutable protocol binding. No API, Docker run, model endpoint, pilot, experiment, push, or PR occurred. |
...
| Transfer checkpoint verification | Remote checkpoint intact; local reliability commits prepared; not pushed or PR-ready | Remote commit `38ad1b3` is exactly based on `origin/main` commit `1d94774` and preserves the 44-file M1-M9 transfer checkpoint. Four local review checkpoints now separate storage/protocol hardening (`b364f09`), pandas 3 compatibility (`810ecb6`), submission-contract hardening (`222ee2e`), and this status update. Across the three code commits, 13 source/test files changed; the previous checkpoint's non-reproduced Jupyter failures passed in focused reruns, and the final full offline suite passed `604 passed, 3 skipped, 2 deselected` in 328.31s. The compatibility audit fixed deterministic handling of pandas `StringDtype` categorical columns and integer-encoded predictions against non-numeric targets; focused coverage passed for both object and pandas string targets. Nothing has been pushed, and the mixed M1-M9 ancestry still must be split into PR-level claim boundaries before any PR. |

Remote checkpoint `38ad1b3` stacks M1b, M2a, M2b, M3, VQ1, M4, M5, M6,
M7, M8, and M9 on M1a. The local branch adds the four committed reliability
checkpoints described above. It must still be split into reviewable PR claim
boundaries; it is not yet a PR-ready lineage. The M2 mechanism
...
| Date | Change |
|------|--------|
| 2026-09-11 | Extended the local reliability audit without activating Paper V2. Direct candidate/finalization reads now reject linked store roots just like enumerate/write paths. A successfully published final directory is no longer reported as rolled back solely because best-effort lock cleanup failed, preventing valid candidate evidence from becoming an orphan outside the registry. Process and Docker prediction contracts now both reject multi-output matrices, and the one-shot hidden gate rejects `NaN`/`Inf` metrics while retaining the consumed attempt. Focused submission/terminal/storage group: `53 passed, 3 skipped`; final full offline suite: `604 passed, 3 skipped, 2 deselected` in 328.31s. Compileall and diff checks pass apart from line-ending notices. Changes were separated into local storage/protocol, pandas-compatibility, submission-contract, and status commits with verified `JapanDino` author/committer identity. No push, PR, freeze, API, Docker run, or experiment activation. |
| 2026-09-11 | Continued Paper V2 reliability audit from remote transfer checkpoint `38ad1b3`. Closed concurrent-writer check/replace races in candidate and protected-finalization publication with candidate-scoped exclusive locks; each real 8-writer test passed 20/20 stress runs. Registry restore now rejects foreign-protocol bundles before incumbent/context admission, and the terminal controller independently rejects them before replay/artifact/hidden evaluation. The first full run exposed two deterministic pandas 3 compatibility defects: native string columns were omitted from categorical/unseen-category profiling, and the current pandas/scikit-learn combination could accept or stringify mixed labels and return an incorrect score before reaching the class-mapping fallback. Both paths now have explicit compatibility handling and regression coverage. Fresh verification: storage/terminal suite `39 passed, 3 skipped`; broader non-Jupyter Paper V2 contracts `182 passed, 3 skipped`; representative notebook integrations `4 passed` in 26.20s; the two previously transient Jupyter terminal/VQ1 targets passed together `2 passed` in 160.53s; final full offline suite `595 passed, 3 skipped, 2 deselected` in 295.48s. Compileall and diff checks pass apart from line-ending notices. No stage, commit, push, PR, freeze, API, Docker run, or experiment activation. |
