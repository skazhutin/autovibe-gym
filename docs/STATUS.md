...
**Last updated:** 2026-06-08 (system-prompt presets end-to-end: refactored SYSTEM_PROMPT into named blocks, dashboard /prompts editor with tier model, MLflow logs preset id/sha/full snapshot per run, /new picker with "modified" badge; gym now beats single-shot)
...
| Date | Change |
|------|--------|
| 2026-06-08 | System-prompt presets end-to-end: `gym/prompts.py` splits the monolithic SYSTEM_PROMPT into named blocks with a tier model (locked / trusted / editable) and a regression test asserting the default assembly is byte-identical to the historical prompt; `dashboard/server/app/services/prompt_store.py` + `/api/prompts` router persist user presets as one JSON file each in `data/prompts/`, with the `default` preset always synthesized from code (a hand-written `default.json` is ignored); locked-block overrides return 422, unknown blocks return 422, sanity check emits non-blocking warnings when contract phrases (restart_and_run_all / validate / submit / model_var / predict / raw) are missing; run launcher resolves the selected preset through `prompt_store.build_runtime_payload` and pipes the assembled prompt to the agent via base64 env var (works for local + SSH); `GymAgent` / `FixedTransitionsAgent` apply overrides through the same `build_system_prompt` path; `run_gym.py` / `run_fixed.py` tag every MLflow run with `prompt_preset_id` / `prompt_sha256` / `prompt_default` and dump the full assembled prompt to `episode_private/system_prompt.txt`; `mlflow_store` surfaces `promptPresetId` / `promptSha256` / `promptIsDefault` on run records (legacy runs get null — never silently labelled "default"); dashboard `/prompts` page renders a two-pane editor with a permanent warning banner, per-block tier badges, trusted-block edit-confirm modal, modified/reset affordances and sanity-warning surfacing; `/new` gets a system-prompt picker with a yellow "modified" badge for non-default presets so the operator never confuses a customised run with the control benchmark |
