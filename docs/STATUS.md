...
| Date | Change |
|------|--------|
| 2026-08-16 | Prepared PR 6 from merged PR #73 (`d1ec571`): added a TMLR-style working manuscript, primary-source bibliography, explicit limitations and AI/API disclosure, reproducibility guide, and SHA-256 artifact manifest. Review hardening byte-validates all 13 canonical publication artifacts, adds a tamper regression, clarifies the combined 256k token cap, and records omitted preregistered resource fields as an incomplete-reporting deviation rather than a post-outcome reconstruction. Pandoc HTML preview rendered; post-review focused suite `21 passed`; full suite `390 passed, 2 skipped`, one existing Jupyter warning. No journal submission, public release, raw-run commit, private registry access, or paid API request. |
