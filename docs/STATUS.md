...
| PR 6 manuscript package | Ready for review | A 3,757-word TMLR-style working manuscript, checked primary-source bibliography, limitations, AI/API disclosure, reproducibility guide, and SHA-256 artifact manifest are under `paper/`. `research.build_manuscript` derives every numeric Results statement/table from the content-hashed analysis, byte-validates all publication artifacts, and detects stale or tampered evidence. Post-review focused suite: `21 passed`; full suite: `390 passed, 2 skipped`, one existing Jupyter warning. No submission, release, raw trajectories, private registry, or paid API request |
