...
All agent code used Docker-backed execution, but the interaction runtime differed by arm. Arm A executed each independent solution script through the Docker `CodeExecutor`; only iterative Arms B and C used a persistent notebook environment. The hidden test split was not mounted into either agent workspace or exposed in observations. Before submission, all arms used the same host-controlled validation contract. Host-side autofitting was disabled: the agent had to fit and retain a candidate capable of predicting raw validation rows after the arm-appropriate clean execution or replay. Candidate prediction and hidden evaluation ran in a separate ephemeral Docker container with networking disabled, a read-only filesystem, and dropped capabilities. Only a validated scalar prediction vector crossed the evaluation boundary.
...
The completeness gate reconciled 120 terminal agent outcomes against 120 planned conditions. There were no pending, running, replacement-required, or infrastructure-censored conditions in the final analysis population.

### 4.1 Primary comparison: iterative feedback versus best-of-N

Arm A achieved a mean FANU of 0.894; Arm B achieved 0.162. The equal-stratum paired difference was -0.731 (95% bootstrap CI [-0.848, -0.609]; two-sided permutation p=0.000010; Holm-adjusted p=0.000020). The interval excludes zero in the direction opposite to the motivating prediction: under this frozen budget and implementation, iterative feedback was worse than budget-matched independent attempts.

Valid submissions occurred in 30/40 Arm A conditions (75.0%, 95% Wilson CI [59.8%, 85.8%]) and 6/40 Arm B conditions (15.0%, 95% Wilson CI [7.1%, 29.1%]). The exact paired McNemar p-value was 0.00000080; 25 pairs were valid only under A and 1 only under B.

### 4.2 Checklist increment

Arm C achieved a mean FANU of 0.113, compared with 0.162 for Arm B. The paired difference was -0.049 (95% CI [-0.211, 0.107]; Holm-adjusted permutation p=0.570312). Thus the study did not detect a clear incremental benefit from checklist feedback. Arm C produced 3/40 valid submissions (7.5%, 95% Wilson CI [2.6%, 19.9%]); the B-versus-C exact McNemar p-value was 0.453125.

### 4.3 Confirmatory effect table

| Hypothesis | Contrast | Pairs | Control mean FANU | Treatment mean FANU | Difference | 95% CI | Holm p |
|---|---:|---:|---:|---:|---:|---:|---:|
| H1 | B - A | 40 | 0.894 | 0.162 | -0.731 | [-0.848, -0.609] | 0.000020 |
| H2 | C - B | 40 | 0.162 | 0.113 | -0.049 | [-0.211, 0.107] | 0.570312 |

![Paired confirmatory effects](../research/publication/paper_v1/primary-effects.svg)

### 4.4 Terminal outcomes and resource use

The terminal category distribution differed sharply by arm. Arm A had 30 successes and 10 invalid submissions. Arm B had 6 successes and 34 budget-exhausted outcomes; Arm C had 3 successes and 37 budget-exhausted outcomes. Budget exhaustion is therefore the dominant observed terminal mechanism for the iterative arms, although the present experiment does not isolate which component of the iterative policy produced it.

| Arm | Valid / 40 | Input tokens | Output tokens | Reasoning tokens | Total protocol tokens | LLM calls |
|---|---:|---:|---:|---:|---:|---:|
| A | 30 | 1,167,206 | 378,165 | 0 | 1,545,371 | 480 |
| B | 6 | 4,158,059 | 137,716 | 0 | 4,295,775 | 474 |
...
For H1, 1 of eight dataset-model strata had a positive mean difference, 1 was exactly zero, and 6 were negative. For H2, the corresponding counts were 1 positive, 4 zero, and 3 negative. These small stratum samples are descriptive rather than separately powered hypothesis tests.

| Hypothesis | Dataset | Model endpoint | Mean paired FANU difference |
|---|---|---|---:|
| H1 | air_quality | deepseek-v4-flash | 0.181 |
| H1 | air_quality | gemma-4-26b | 0.000 |
| H1 | bank_marketing | deepseek-v4-flash | -1.698 |
| H1 | bank_marketing | gemma-4-26b | -1.013 |
| H1 | diabetes_readmission | deepseek-v4-flash | -0.973 |
| H1 | diabetes_readmission | gemma-4-26b | -0.580 |
| H1 | student_dropout | deepseek-v4-flash | -0.980 |
| H1 | student_dropout | gemma-4-26b | -0.786 |
| H2 | air_quality | deepseek-v4-flash | -0.181 |
| H2 | air_quality | gemma-4-26b | 0.000 |
| H2 | bank_marketing | deepseek-v4-flash | 0.000 |
| H2 | bank_marketing | gemma-4-26b | 0.187 |
| H2 | diabetes_readmission | deepseek-v4-flash | 0.000 |
| H2 | diabetes_readmission | gemma-4-26b | -0.197 |
| H2 | student_dropout | deepseek-v4-flash | 0.000 |
| H2 | student_dropout | gemma-4-26b | -0.201 |

Checklist coverage is intentionally absent from the results: the preregistered two-annotator detector validation has not been completed, so detector outputs are not treated as validated evidence.
